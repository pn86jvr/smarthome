...
public class WeatherUndergroundHandler extends BaseThingHandler {
    ...
    private Logger logger = LoggerFactory.getLogger(WeatherUndergroundHandler.class);
    ...
    public WeatherUndergroundHandler(Thing thing) {
        super(thing);
    ...
    public void initialize() {
        ...
        if (validConfig) {
            ...
            logger.info("Disabling thing '{}': {}", getThing().getUID(), errors);
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        if (command instanceof RefreshType) {
            ...
            logger.info("The Weather Underground binding is a read-only binding and can not handle command {}",
                    command);
        }
    }
    ...
    private WeatherUndergroundJsonData getWeatherData(Set<String> features, String location) {
        ...
        try {
            ...
            urlStr = urlStr.replace("%FEATURES%", featuresStr.substring(1));
            ...
            logger.debug("weatherData = {}", repsonse);

            // Map the JSON response to an object
            result = gson.fromJson(repsonse, WeatherUndergroundJsonData.class);
            if (result == null) {
            ...
            error = "Constructed url is not valid";
